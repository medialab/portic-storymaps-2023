...
const CampFire = ({
  ...
  const { r } = useSpring({
    loop: true,
    to: [{ r: baseRadius }, ...fireSequence, { r: baseRadius }],
    from: { r: baseRadius },
    config: { duration: 100 }
  });
  ...
  return (
    <>
      <animated.circle
        ...
        cy={y}
        r={r}
        fill={`url(#night-fire-${id})`}
        opacity={hourOfTime >= 20 || hourOfTime <= 6 ? 1 : 0}
      />
      <radialGradient id={`night-fire-${id}`}>
...
const Boat = ({
  ...
  const campFires = useMemo(() => {
    const numberOfFires = Math.round(1 + Math.random() * 10);
    let f = [];
    const hullHeight = height * hullHeightRatio;
    for (let i = 0; i < numberOfFires; i++) {
      const x = boatWidth * .1 + Math.random() * boatWidth * .8;
      let y = height - hullHeight * .5;
      if (x < boatWidth / 3 || x > boatWidth / 3 * 2) {
        y = height - hullHeight;
      }
      f.push({
        x,
        y,
        baseRadius: boatWidth / 30 + Math.random() * boatWidth / 20
      })
    }
    return f;
  }, [boatWidth, hullHeightRatio, height])
  ...
  return (
    ...
    >
      {
        campFires.map(({ x, baseRadius, y }, index) => (
          <CampFire
            key={index}
            {
            ...{
